...
barplot(table(ds$sex), main = '성별 분포', col = colors[5:6])
barplot(table(ds$education), main = '교육수준별 분포', col = colors[7:9])
...
barplot(table(tips$time), main = '시간대별', col = color.2[2:1]) 
...
hist(tips.new$perHead, breaks = 5, main = '1인당 주문액',
     xlab = '단위:$', ylab = '빈도수', las = 1, col = color.5)
hist(tips.new[tips.new$time == 'Lunch', 'perHead'], breaks = 4, main = '1인당 주문액(점심)',
     xlab = '단위:$', ylab = '빈도수', las = 1, col = color.5)
hist(tips.new[tips.new$time == 'Dinner', 'perHead'], breaks = 5, main = '1인당 주문액(저녁)',
     xlab = '단위:$', ylab = '빈도수', las = 1, col = color.5)
...
res.lunch <- hist(tips.new[tips.new$time == 'Lunch', 'percent'], 
                  main = '주문액 대비 팁비율(점심)', breaks = 5,
                  xlab = 'per(%)')
res.dinner <- hist(tips.new[tips.new$time == 'Dinner', 'percent'], 
                  main = '주문액 대비 팁비율(저녁)', breaks = 5,
                  xlab = 'per(%)')
...
barplot(res, main = '식사시간에 따른 팁비율 분포',
        col = color.2, xlab = '분',
        legend.text = c('Lunch', 'Dinner'), las = 1)
